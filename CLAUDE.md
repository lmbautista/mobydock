...
**Machine lifecycle:** `launch`/`start` create or start the `docker-machine`, `shutdown` stops the running instance (`docker-machine stop`, idempotent via a status check; refused for protected envs in `perform_shutdown` via `Helpers.shutdown_protected` unless `--force`, so only non-protected envs like staging can be stopped freely), `destroy` removes it (`docker-machine rm -y`, which also terminates the remote EC2 instance). `destroy` is gated on `--force` in `perform_destroy` (returns `Helpers.destroy_protected` otherwise). `mobydock ls` is a top-level, env-less command handled by `machine_ls?` in `Runner#call` (before the `docker_running?` check) that maps to `docker-machine ls`.
